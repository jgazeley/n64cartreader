...
static bool cdc_read_with_timeout(uint8_t *b, uint32_t timeout_ms) {
    ...
    while ((time_us_32() - start_time) < timeout_us) {
        ...
        tud_task(); // Keep the USB stack alive while we wait
    }
    return false; // Stopwatch ran out
}
...
static const transport_t cdc_backend = {
    ...
    .read_with_timeout = cdc_read_with_timeout,
};
